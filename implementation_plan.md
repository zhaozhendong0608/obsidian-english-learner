...
## 全局 UI 架构与布局重构方案 (2026-05-24 增补)

### 变更目标
重构侧边栏面板（`src/ui/Panel.vue`）的布局结构。将**全局自主查词输入框**、**发音配置面板 (⚙️)** 和**选中单词详情卡片 (selectedWord)** 提到 Tab 切换区域的顶部，让所有 Tab 共享这套输入与详情状态，大幅优化视觉高度和查词体验的流畅度。

### Pizza Slicing 计划
*   **S1-Bone (骨架布局重构)**：
    *   修改 `src/ui/Panel.vue` 中的 `<template>`。
    *   移除 Tab 3（整句分析）内部的 `<div class="lang-learner-voice-settings-section">`，将其放置到最顶层的查词框下方。
    *   将 `v-if="selectedWord"` 的单词详情框也移动到查词框和发音配置的下方，但依然在 `lang-learner-main-tabs` 的上方。
*   **S2-Muscle (样式与联动优化)**：
    *   微调发音设置在全局顶部浮动展现时的折叠状态样式，确保不占用过多的垂直空间。
    *   测试各 Tab 间切换时全局发音参数和选词状态的持久度。

### 验证计划
1.  **编译验证**：运行 `npm run build`，确保无模板或属性绑定语法报错。
2.  **交互验证**：
    *   点击“整句分析”或其他 Tab，确认顶部始终保留“自主查词”和“发音配置”；
    *   在任何 Tab（如词汇测试）中选中或输入单词，顶部能立刻渲染单词详情卡片且不影响下方 Tab 内部的状态流转；
    *   折叠/展开全局发音配置，确认它不会被局部 Tab 遮挡。

---

## 中文查词与搜索结果列表实现方案 (2026-05-24 增补)

### 变更目标
支持在顶部查词框输入中文进行检索。如果匹配到多个英文单词，单词详情区域自动改变为单词列表（搜索结果列表），显示匹配的英文词和释义；点击列表中的任一英文词可展示该词的详细卡片，且卡片中提供“返回列表”按钮，方便用户返回搜索结果。

### 核心实现逻辑
1. **中文检索与去重合并**：
   - 检测输入字符是否包含中文：`const hasChinese = /[\u4e00-\u9fa5]/.test(query);`。
   - 若包含中文，同时扫描内存词库 `vocabManager.getAllEntries()` 以及本地内置离线词典 `OFFLINE_DICT`。
   - 过滤所有翻译包含该中文的英文单词原型，存入数组去重后进行上限切片（最大 100 个结果以防止过多渲染），更新 `searchResultsList`。
   - 如果只匹配到 1 个英文单词，则直接展示详情（与现有英文单词查词表现一致）；如果匹配到 0 个，使用 Notice 友好提示。
2. **列表-详情联动机制**：
   - 增加响应式状态 `searchResultsList` (String 数组)。
   - 在 `performSearch` 逻辑中：若是英文搜索，清空 `searchResultsList` 并正常查词原型；若是中文搜索且多于 1 个结果，清空 `selectedWord` 并将匹配的单词存入 `searchResultsList`。
