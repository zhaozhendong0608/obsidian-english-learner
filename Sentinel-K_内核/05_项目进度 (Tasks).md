...
## 🎙️ 阶段 10: 发音评测端侧集成与多端兼容优化 [已完成]
- [x] 集成 `onnxruntime-web` 并配置 Web Worker 动态加载端侧 `whisper-tiny-en`模型 `(2026-05-29)`
- [x] 设计 Vault 内轻量模型文件的安全存储、路径解析与首屏加载进度指示器 `(2026-05-29)`
- [x] 优化音素强制对齐算法（G2P），引入离线音标词典（如 CMU 压缩版）提升对齐准确度 `(2026-05-29)`
- [x] 利用 Web Audio API (`AnalyserNode`) 为 `PronunciationTab.vue` 实现录音实时波形图反馈 `(2026-05-29)`
- [x] 增加多端（iOS/Android/Desktop）麦克风录音权限检测与友好排障兜底机制 `(2026-05-29)`
- [x] 实现 AI 发音诊断和 AI 教师详细解析的 SSE 流式响应展示与渐进式 Markdown 渲染 `(2026-05-29)`

## ✍️ 阶段 11: 中译英写作增强与反向检索联想 [已完成]
- [x] 在静态生成脚本中构建”反向离线词典索引”，实现中文词汇到英文 Lemma 的离线多对多映射 `(2026-05-27)`
- [x] 升级查词输入框，增加中/英自适应输入判别，支持输入中文时自动触发本地反向索引检索 `(2026-05-27)`
- [x] 打通翻译候选词与影子生词库熟悉度状态（KNOWN/LEARNING/UNKNOWN），支持一键批量收藏 `(2026-05-27)`
- [x] 升级 AI 教师提示词模板，支持输入中文时的”同义词精细语境辨析”与场景造句 `(2026-05-29)`
- [x] 编写汉译英写作联想补全（`WordSuggest` 扩展），支持通过特定前缀（如 `::掌握`）在编辑器内唤起中文联想推荐英文候选词 `(2026-05-29)`

## 👅 阶段 12: 发音音素口腔动画可视化 [已完成]
- [x] 编写 `src/utils/phonemeVisuals.ts` 定义音素映射与 12 类口腔变形状态的矢量路径坐标数据 `(2026-05-29)`
- [x] 升级 `src/ui/components/PronunciationTab.vue`，实现音素点击交互、SVG 矢量形态绑定与纠偏文字说明渲染 `(2026-05-29)`
- [x] 升级 `src/styles.css`，配置矢状图主题自适应色值、CSS 路径变形平滑过渡和气流流动流动特效 `(2026-05-29)`
- [x] 进行编译构建回归，确保 vitest 全绿并通过 Obsidian 调试沙盒验证 `(2026-05-29)`
