...
| ADR-011 | 2026-05-23 | 离线语音合成发音（TTS）核心逻辑与侧边栏深度绑定 | 🟢 Accepted | -                  |
...
### ADR-011 离线语音合成发音（TTS）核心逻辑与侧边栏深度绑定
- **上下文**:
  原本的侧边栏只提供了纯文本形式的词汇列表、翻译结果以及生词详情，缺失对单词和句子的语音朗读（发音）功能。为提供更立体的英语精读学习环境，需要为全句翻译卡片、全局单词详情、句中词汇清单及生词本列表提供即时发音支持，且发音引擎必须符合插件“离线优先/轻量响应”的体系设计。
- **决策**:
  1. **Web Speech API 离线语音合成选型**：废弃依赖网络连接且在无网或受限网络下易失效的在线 TTS 音频流 API，引入浏览器标准的 `window.speechSynthesis` 接口，调用操作系统原生级音库（如 macOS Siri 音库）实现 100% 离线、毫秒级响应的美音发音；
  2. **speak 播放打断隔离设计**：定义 `speak(text)` 全局公用播放逻辑，每次调用时先强制触发 `speechSynthesis.cancel()` 强行斩断当前的发音通道，隔离前后连续快速点击产生的混音干扰；
  3. **多模态发音按钮绑定**：在 `Panel.vue` 中将 `speak` 方法暴露到模版，深度绑定并渲染 4 处小喇叭 `🔊` 按钮，包含整句翻译朗读、选中词汇详情、分析清单单字发音和生词本快捷发音，通过 `@click.stop` 阻断祖先节点点击事件。
- **后果**:
  - 正面：实现了 100% 离线、零延迟的高质量整句与单字朗读体验；打断隔离机制避免了发音重叠混杂；多处按钮绑定提高了交互友好度；
  - 负面：语音质量受限于用户宿主操作系统自带的 TTS 英文音库渲染质量（但现代 OS 均自带高拟真音库，影响极小）。
