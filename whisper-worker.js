  ...
  var La = V(() => {
    ...
    Na = (t, e) => {
      ...
      if (r != null) {
        ...
        let m = i * o;
        if (e !== void 0 && (e.format !== void 0 && s === 4 && e.format !== "RGBA" || s === 3 && e.format !== "RGB" && e.format !== "BGR"))
...
`))[0] == "Error" && f.shift(), Ci(f);
    ...
    var On, zn = {}, Ai = () => {
      if (!On) {
        ...
        for (a in zn)
          zn[a] === void 0 ? delete l[a] : l[a] = zn[a];
        var h = [];
        for (a in l)
          h.push(`${a}=${l[a]}`);
        On = h;
...
`, Ag = (t, e, r) => (() => {
  ...
  ${m.mainStart()}
    ...
    qkv_with_bias[global_idx] = qkv[global_idx] + bias[bias_offset_idx];
      ...
      if (m)
        return Wt(t, g, o, i, u, void 0, c, p, d, r);
      if (!o || !i)
...
${u}`, c = n.createShaderModule({ code: d, label: e.name });
  ...
  var WorkerStatus = /* @__PURE__ */ ((WorkerStatus2) => {
    WorkerStatus2["LOADING"] = "loading";
    WorkerStatus2["READY"] = "ready";
    WorkerStatus2["PROCESSING"] = "processing";
  ...
  var workerStatus = "loading" /* LOADING */;
  ...
  function sentenceToPhonemes(text) {
  ...
  function getMelFilterbank(numMelBins = 80, fftSize = 512, sampleRate = 16e3) {
    ...
    for (let m = 1; m <= numMelBins; m++) {
      ...
      for (let k2 = left; k2 < center; k2++) {
        filterbank[m - 1][k2] = (k2 - binPoints[m - 1]) / (binPoints[m] - binPoints[m - 1]);
      }
      for (let k2 = center; k2 <= right; k2++) {
        filterbank[m - 1][k2] = (binPoints[m + 1] - k2) / (binPoints[m + 1] - binPoints[m]);
      }
    }
    return filterbank;
  }
  function computeLogMelSpectrogram(audioData) {
    const numMelBins = 80;
    const fftSize = 512;
    const winLength = 400;
    const hopLength = 160;
    ...
    const filterbank = getMelFilterbank(numMelBins, fftSize, 16e3);
    ...
    for (let frame = 0; frame < maxFrames; frame++) {
      ...
      for (let k2 = 0; k2 < powerSpec.length; k2++) {
        powerSpec[k2] = real[k2] * real[k2] + imag[k2] * imag[k2];
      }
      for (let melBin = 0; melBin < numMelBins; melBin++) {
        let melVal = 0;
        for (let k2 = 0; k2 < powerSpec.length; k2++) {
          melVal += powerSpec[k2] * filterbank[melBin][k2];
        }
        const logMelVal = Math.log10(Math.max(melVal, 1e-5));
        const index = melBin * maxFrames + frame;
        melSpectrogram[index] = (logMelVal + 4) / 4;
      }
    }
    return melSpectrogram;
  }
  function alignPhonemes(targetPhonemes, detectedPhonemes) {
    ...
    for (let i2 = 1; i2 <= n; i2++) {
      for (let j2 = 1; j2 <= m; j2++) {
        if (targetPhonemes[i2 - 1] === detectedPhonemes[j2 - 1]) {
  ...
  self.onmessage = async (event) => {
    ...
    try {
      if (request.type === "init") {
        ye.wasm.numThreads = 1;
        if (request.wasmBinary) {
          console.log("[WhisperWorker] \u6CE8\u5165\u4E3B\u7EBF\u7A0B\u4F20\u9012\u7684 WASM \u5185\u5B58\u4E8C\u8FDB\u5236\uFF0C\u907F\u5F00 CORS \u9650\u5236");
          ye.wasm.wasmBinary = request.wasmBinary;
        } else if (request.wasmPaths) {
          ye.wasm.wasmPaths = request.wasmPaths;
        }
        if (request.encoderBuffer && request.decoderBuffer) {
          console.log("[WhisperWorker] \u6B63\u5728\u5B9E\u4F8B\u5316 ONNX \u63A8\u7406 Session...");
          encoderSession = await vf.create(request.encoderBuffer);
          decoderSession = await vf.create(request.decoderBuffer);
        ...
        if (encoderSession && decoderSession) {
          try {
            ...
            const encoderTensor = new Ke("float32", logMel, [1, 80, 3e3]);
            ...
            for (let step = 0; step < maxLen; step++) {
              ...
              const tokenTensor = new Ke("int64", BigInt64Array.from(currentTokens.map(BigInt)), [1, currentTokens.length]);
              ...
              if (decoderSession.inputNames.includes("use_cache_branch")) {
                feeds["use_cache_branch"] = new Ke("bool", [step > 0], [1]);
              }
              for (const inputName of decoderSession.inputNames) {
                if (inputName.startsWith("past_key_values.")) {
                  if (step === 0) {
                    feeds[inputName] = new Ke("float32", new Float32Array(0), [1, 6, 0, 64]);
                  } else {
              ...
              const logits = decoderOutputs.logits.data;
              const vocabSize = 51865;
              const currentInputLength = currentTokens.length;
              const lastTokenOffset = (currentInputLength - 1) * vocabSize;
              let maxVal = -Infinity;
              let nextToken = -1;
              for (let k2 = 0; k2 < vocabSize; k2++) {
                if (logits[lastTokenOffset + k2] > maxVal) {
                  maxVal = logits[lastTokenOffset + k2];
                  nextToken = k2;
                }
              }
              if (nextToken === 50257 || nextToken === -1)
                break;
        ...
        const overallScore = alignments.length > 0 ? Math.round(scoreCount / alignments.length * 100) : 0;
